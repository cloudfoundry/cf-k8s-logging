...
pushd "${SCRIPT_DIR}" > /dev/null
  ...
  kbld -f "${KBLD_CONFIG_DIR}" -f "${SCRIPT_DIR}/fluentbit-image.yml" -f <(ytt -f "${SCRIPT_DIR}/../config/" -f "${SCRIPT_DIR}/sample-logging-install-values.yml") --lock-output "${KBLD_LOCK_FILE}"
popd > /dev/null
